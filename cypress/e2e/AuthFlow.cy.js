...
const SELECTORS = {
  ...
  existingEmailMessage:
    ".basis-0 > .justify-start > .flex > div > :nth-child(1)",
...
const loginToAccount = () => {
  cy.get(SELECTORS.websiteEmail).click().type(TEST_DATA.existingEmail);

  cy.get(SELECTORS.websitePassword).click().type(TEST_DATA.password);

  cy.get(SELECTORS.submitButton).click();

  cy.get(SELECTORS.loginElement).should("be.visible");
};
// Const used to logout from an account
const logOutToAccount = () => {
  cy.get(SELECTORS.userMenu).click();

  cy.get(SELECTORS.userMenuItem).eq(1).click();
...
const forgetExistingAccount = () => {
  cy.get(SELECTORS.forgetPassword).click();
...
const forgetNonExistingAccount = () => {
  ...
  cy.get(SELECTORS.websiteEmail).click().type(TEST_DATA.nonExistentEmail);
